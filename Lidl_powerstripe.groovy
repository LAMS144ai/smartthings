 ...
 *  Date : 2018-08-29
*   Mod Version for Lidl Power Stripe CH 
 */

metadata {
	definition(name: "Lidl Power Stripe", namespace: "smartthings", author: "SmartThings", ocfDeviceType: "oic.d.smartplug", mnmn: "SmartThings", vid: 
...
"generic-switch-power") {
        ...
        // LIDL
		fingerprint profileId: "0104", inClusters: "0000, 0003, 0004, 0005, 0006", outClusters: "000A, 0019", manufacturer: "_TZ3000_1obwwnmq", 

model: "TS011F", deviceJoinName: "Lidl Stripe Plug 1"
...
"turningOn"
				attributeState "turningOn", label: '${name}', action: "switch.off", icon: "st.switches.light.on", backgroundColor: "#00A0DC", 
		...
		if (eventDescMap && eventDescMap?.attrId == "0000") {//0x0000 : OnOff attributeId
			if (eventDescMap?.sourceEndpoint == "01" || eventDescMap?.endpoint == "01") {
				...
				def childDevice = childDevices.find {
					it.deviceNetworkId == "$device.deviceNetworkId:${eventDescMap.sourceEndpoint}" || it.deviceNetworkId == 

"$device.deviceNetworkId:${eventDescMap.endpoint}"
				}
				if (childDevice) {
...
def configureHealthCheck() {
	...
	if (!state.hasConfiguredHealthCheck) {
		...
		def healthEvent = [name: "checkInterval", value: hcIntervalMinutes * 60, displayed: false, data: [protocol: "zigbee", hubHardwareId: 

device.hub.hardwareID]]
